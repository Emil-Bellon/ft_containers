...
namespace ft
{
	...
	/* is_integral */
	template<class T>		struct remove_c				{ typedef T type; };
	template<class T>		struct remove_c<const T>	{ typedef T type; };
	...
	bool equal(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2, BinaryPredicate binary_pred)
	{
		while (first1 != last1 && first2 != last2)
