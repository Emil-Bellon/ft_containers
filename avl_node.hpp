...
namespace ft
{
	...
	class avl_node
	{
		...
		public :
			...
			static node_ptr right_rotate(node_ptr y)
			{
				...
				y->_height =	ft::max(height(y->left), height(y->right)) + 1;
				x->_height =	ft::max(height(x->left), height(x->right)) + 1;
	...
	class avl_tree_iterator : public ft::map_iterator<std::bidirectional_iterator_tag, ft::pair<const Key, _Tp> >
	{
	...
	template<class Key, class _Tp, class Compare = ft::less<Key>, class Allocator = std::allocator< ft::pair<const Key, _Tp> > >
	class avl_tree_const_iterator : public ft::map_iterator< std::bidirectional_iterator_tag, ft::pair<const Key, _Tp> >
	{
		...
		public :
			...
			avl_tree_const_iterator(const avl_tree_const_iterator & other)	: _root(other._root), _current(other._current)	{}
